 ...
 */
public class Restauracie extends Fragment {

    private List<CardPodnik> restauracie_list;
    ...
    public View onCreateView(LayoutInflater inflater,final ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_restauracie, container, false);

        restauracie_list = new ArrayList<>();
        podniky_view_list = view.findViewById(R.id.restauracia_list);

        firebaseAuth = FirebaseAuth.getInstance();

        podnikRecyclerAdapter = new PodnikRecyclerAdapter(restauracie_list);
        ...
        if(firebaseAuth.getCurrentUser() != null) {
            ...
            * */
         //   Query firstQuery = firebaseFirestore.collection("podniky").whereEqualTo("typ","Reštaurácia").orderBy( "countOblubene", Query.Direction.DESCENDING).limit(4);
                    ...
                    if (!documentSnapshots.isEmpty()) {
                        ...
                        for (DocumentChange doc : documentSnapshots.getDocumentChanges()) {
                            ...
                            if (doc.getType() == DocumentChange.Type.ADDED) {
                                ...
                                if(cardPodnik.getTyp().contains("Reštaurácia")){

                                    if (isFirstPageFirstLoad) {

                                        restauracie_list.add(cardPodnik);

                                    } else {

                                        restauracie_list.add(0, cardPodnik);
    ...
    public void loadMorePost(){
        ...
        if(firebaseAuth.getCurrentUser() != null) {
           ...
           // Query nextQuery = firebaseFirestore.collection("podniky").whereEqualTo("typ","Reštaurácia").startAfter(lastVisible).limit(4);
