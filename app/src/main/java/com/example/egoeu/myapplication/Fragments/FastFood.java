...
public class FastFood extends Fragment {
    ...
    private List<CardPodnik> fast_food_list;
    ...
    public View onCreateView(LayoutInflater inflater,final ViewGroup container,
        ...
        fast_food_list = new ArrayList<>();
        podniky_view_list = view.findViewById(R.id.fastfood_list);

        firebaseAuth = FirebaseAuth.getInstance();

        podnikRecyclerAdapter = new PodnikRecyclerAdapter(fast_food_list);
                    ...
                    if (!documentSnapshots.isEmpty()) {
                        ...
                        for (DocumentChange doc : documentSnapshots.getDocumentChanges()) {
                            ...
                            if (doc.getType() == DocumentChange.Type.ADDED) {
                                ...
                                if(cardPodnik.getTyp().contains("Fast food")) {

                                    if (isFirstPageFirstLoad) {

                                        fast_food_list.add(cardPodnik);

                                    } else {

                                        fast_food_list.add(0, cardPodnik);
